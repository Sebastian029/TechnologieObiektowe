...
class Neo4jConverter:
    ...
    def _recursive_save(self, obj, parent=None, rel_type=None):
        if id(obj) in self._saved_nodes:
            ...
            if self._is_basic_type(value):
                ...
                for item in value:
                    if not self._is_basic_type(item):
                        self._recursive_save(item, obj, attr.upper())
            elif isinstance(value, dict):
                for item in value.values():
                    if not self._is_basic_type(item):
                        self._recursive_save(item, obj, attr.upper())
            elif hasattr(value, "__dict__"):
                self._recursive_save(value, obj, attr.upper())
